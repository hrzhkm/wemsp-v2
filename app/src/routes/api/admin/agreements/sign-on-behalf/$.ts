...
import { ensureAgreementMintedWithMetadata } from '@/lib/agreement/agreement-metadata'
import {
  getAgreementData,
...
export const adminSignOnBehalfHandlers = {
  OPTIONS: async () => {
    ...
    try {
      ...
      if (!beneficiary.nonRegisteredFamilyMemberId) {
        return Response.json(
          {
            error:
              'Only non-registered family members can be signed by admin proxy',
          },
          { status: 400, headers: corsHeaders },
        )
      }
      ...
      const beneficiaryRecordId = String(beneficiary.id)
      const onChainSignatureStatus = await getBeneficiarySignatureStatus(
        tokenId,
        beneficiaryRecordId,
      )
...
export const Route = createFileRoute('/api/admin/agreements/sign-on-behalf/$')({
  server: {
    handlers: adminSignOnBehalfHandlers,
  },
})
