...
const METADATA_BASE_URI =
  process.env.AGREEMENT_METADATA_BASE_URI || 'wemsp://agreement'
const EXPLORER_TX_BASE =
  process.env.BLOCK_EXPLORER_TX_BASE || 'https://sepolia.basescan.org/tx'
const EXPLORER_ADDRESS_BASE =
  process.env.BLOCK_EXPLORER_ADDRESS_BASE ||
  'https://sepolia.basescan.org/address'

const RETRY_LIMIT = Number(process.env.ONCHAIN_RETRY_LIMIT || 4)
const RETRY_BASE_DELAY_MS = Number(
  process.env.ONCHAIN_RETRY_BASE_DELAY_MS || 500,
)
...
export function getProvider(): JsonRpcProvider {
  ...
  if (!_provider) {
    ...
    _provider = new JsonRpcProvider(RPC_URL, staticNetwork, {
...
export function getFallbackProvider(): JsonRpcProvider | null {
  ...
  if (!_fallbackProvider) {
    const staticNetwork = new Network(CHAIN_NAME, CHAIN_ID)
    _fallbackProvider = new JsonRpcProvider(RPC_URL_FALLBACK, staticNetwork, {
      pollingInterval: 4000,
    })
  }
...
async function signatureResultFromReceipt(
  ...
  const block = await withRetry(() =>
    getProvider().getBlock(receipt.blockNumber),
  )
  return {
    txHash: receipt.hash,
...
export async function recordOwnerSignature(
  tokenId: number,
): Promise<SignatureResult> {
  const contract = getContract()
  const { receipt } = await sendAndConfirmReceipt(() =>
    contract.recordOwnerSignature(tokenId),
  )
...
export async function recordWitnessSignature(
  ...
  const contract = getContract()
  const { receipt } = await sendAndConfirmReceipt(() =>
    contract.recordWitnessSignature(tokenId),
  )
  return signatureResultFromReceipt(receipt)
}
...
export async function finalizeAgreement(
  ...
  const contract = getContract()
  const { receipt } = await sendAndConfirmReceipt(() =>
    contract.finalizeAgreement(tokenId),
  )
  return signatureResultFromReceipt(receipt)
}
...
export async function getAgreementData(
  ...
  const data = await readWithFallback((contract) =>
    contract.getAgreement(tokenId),
  )
...
export async function getTokenIdByAgreementId(
  agreementId: string,
): Promise<number> {
...
export async function getTokenIdByVisibleId(
  visibleId: string,
): Promise<number> {
...
export {
  getErrorMessage as getOnChainErrorMessage,
  getTimestampDate as getOnChainTimestampDate,
}
