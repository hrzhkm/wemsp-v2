...
export const agreementHandlers = {
      ...
      if (idParam !== 'agreement') {
        ...
        const transformedAgreement = {
          ...
          beneficiaries: agreement.beneficiaries.map((ab) => ({
            ...
            explorerUrl: ab.signatureRef
              ? getExplorerUrl(ab.signatureRef)
              : null,
  ...
  POST: async ({ request }: { request: Request }) => {
    ...
    try {
      ...
      if (
        !distributionType ||
        !Object.values(DistributionType).includes(distributionType)
      ) {
        return Response.json(
        ...
        return Response.json(
          {
            ...
            details: beneficiaryValidation.errors,
          },
          { status: 400 },
        )
      }
    ...
    try {
        ...
        data: {
          ...
          description:
            description !== undefined
              ? description
              : existingAgreement.description,
          distributionType:
            distributionType ?? existingAgreement.distributionType,
          effectiveDate:
            effectiveDate !== undefined
              ? new Date(effectiveDate)
              : existingAgreement.effectiveDate,
          expiryDate:
            expiryDate !== undefined
              ? new Date(expiryDate)
              : existingAgreement.expiryDate,
        },
      })
      ...
      if (existingAgreement.tokenId != null && isContractConfigured()) {
        try {
          ...
          console.error(
            'Error refreshing on-chain agreement metadata:',
            metadataError,
          )
        }
      }
    ...
    try {
      ...
      if (agreement.status !== 'DRAFT') {
        return Response.json(
          {
            error:
              'Only DRAFT agreements can be deleted. Use cancel for other statuses.',
          },
          { status: 403 },
        )
      }
